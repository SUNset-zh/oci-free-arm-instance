...
"$FFMPEG" -y -loglevel error -i "$OUT/video_only.mp4" -i "$OUT/soundtrack.wav" \
  -c:v copy -af loudnorm=I=-15:TP=-1.5:LRA=11 -ar 48000 -c:a aac -b:a 192k -shortest -movflags +faststart \
  "$OUT/crypto_explainer_1080x1920.mp4"
echo "done -> $OUT/crypto_explainer_1080x1920.mp4"
